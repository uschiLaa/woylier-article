...
\abstract{%
The woylier package implements alternative method for interpolation path
between tour frames using Givens rotation.
}
...
\citep{grandtour1995} and it is implemented in the \CRANpkg{tourr}
...
\CRANpkg{tourr} package uses geodesic interpolation between planes.
...
However, in some cases of non-linear projection pursuit index, the
...
Current implementation of \CRANpkg{tourr} package is locally shortest
...
\includegraphics{woylier-article_files/figure-latex/unnamed-chunk-11-1} \end{Schunk}
