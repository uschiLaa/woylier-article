...
proj_2d <- proj_2d %>% 
  mutate(type="torus")
pt_geo <- save_history(flea[, 1:6], planned_tour(list(base1, base2)))
int_geo <- interpolate(pt_geo)
path_geo <- t(apply(int_geo, 3, c)) %>% 
  as.data.frame() %>% 
  mutate(type="geodesic")

base2_rot <- matrix(ncol = 12, nrow = 201)
i <- 1
...
rot_base2 <- as.data.frame(base2_rot) %>% 
  mutate(type="rotated target")
proj_path <- bind_rows(proj_2d, path_2d, path_geo, rot_base2)
p1 <- mutate(proj_path[,c(1:6, 13)], proj = 1)
p2 <- mutate(proj_path[,c(7:12, 13)], proj = 2)
colnames(p2) <- c(paste0("V", 1:6), "type", "proj")
proj_path <- rbind(p1, p2)
proj_path$type <- factor(proj_path$type, levels=c("torus", "target",
                                               "givens", "geodesic"))
proj_path <- arrange(proj_path, desc(type))
animate_xy(proj_path[,1:6], col=proj_path$type, palette="Teal-Rose", 
           cex=c(rep(0.5, 2*n), rep(1, nrow(proj_path)-2*n)),
                 axes="off")
