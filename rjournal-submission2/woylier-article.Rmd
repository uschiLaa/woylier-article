...
The procedure starts with forming an orthonormal basis by applying Gram-Schmidt to $F_z$ with regards to $F_a$: $F_\star = F_z - F_a(F_a^\top F_a)^{-1}F_aF_z$, followed by orthonormalizing subsequent columns of $F_\star$ against all preceding columns. (The function `orthonormalise_by()` in the \CRANpkg{tourr} does this.) Then we build the preprojection basis $B$ by combining $F_a$ and $F_\star$ as follows:
...
1. Construct preprojection basis $B$ by orthonormalizing $F_z$ with regards tp $F_a$ with Gram-Schmidt (same procedure as described above).
